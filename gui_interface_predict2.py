...
def bathroom_trends_updated(from_year, to_year):
    ...
    fig_house, ax_house = plt.subplots(figsize=(20, 6))
    sns.boxenplot(x=date_fil_df['bathrooms'], y=date_fil_df['price'])
    ax_house.set_xlabel('Boxen Plot of ' + str(date_fil_df['bathrooms']))
    ax_house.set_ylabel('Target')
    # ax_house.set_title('Year Built with House Count')
    ax_house.set_title('Period From ' + str(from_year) + ' To ' + str(to_year) )
...
def house_trends_page(from_year, to_year):
    ...
    filtered_df = df[(df['yr_built'] >= from_year) & (df['yr_built'] <= to_year)]

    

    # House trends
    fig_house, ax_house = plt.subplots(figsize=(18, 8))
    sns.countplot(x='yr_built', data=filtered_df, palette='viridis', ax=ax_house)
    ax_house.set_xlabel('Year Built')
    ax_house.set_ylabel('House Count')
    # ax_house.set_title('Year Built with House Count')
    ax_house.set_title('Period From ' + str(from_year) + ' To ' + str(to_year) )
    ax_house.tick_params(axis='x', rotation=90)
    st.pyplot(fig_house)
...
def main():
    ...
    if page == "Main Menu":
        ...
        from_year = st.selectbox("From Year", options=range(1900, 2024), index=0)
        to_year = st.selectbox("To Year", options=range(1900, 2024), index=len(range(1900, 2024))-1)
        city_trends_updated(from_year, to_year)
    elif page == "Bathroom Trends":
        from_year = st.selectbox("From Year", options=range(1900, 2014), index=0)
        to_year = st.selectbox("To Year", options=range(1900, 2014), index=len(range(1900, 2014))-1)
        ...
        st.title("House Trends")
        from_year = st.selectbox("From Year", options=range(1950, 2014), index=0)
        to_year = st.selectbox("To Year", options=range(1950, 2014), index=len(range(1950, 2014))-1)
        if validate_year(str(from_year)) or  validate_year(str(to_year)):
            if from_year >= to_year:
                st.error("From Year must be less than To Year.")
            else:
